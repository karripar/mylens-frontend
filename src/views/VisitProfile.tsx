...
const VisitProfile = () => {
  ...
  const [visitedUser, setVisitedUser] = useState<UserWithNoPassword | null>(null);
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      const response: UserWithNoPassword = await fetchData(import.meta.env.VITE_AUTH_API + '/users/byusername/' + username);
  ...
  return (
    ...
    <div className="flex flex-col items-center justify-center min-h-2/3 p-6">
      <div className="w-full max-w-lg bg-white shadow-2xl rounded-3xl p-8 flex flex-col items-center space-y-6">
        ...
        {/* Username */}
        <h2 className="text-3xl font-bold text-gray-800">{visitedUser.username}</h2>
